...
torch_geometric.utils.add_self_loops它设计为返回一个包含两个元素的 Python 元组 (Tuple)：
元组的第 0 个元素是修改后的 edge_index（维度变为 [2, E + N]）。
元组的第 1 个元素是自动调整后的 edge_attr（如果输入了的话）。

        添加自环 (Add Self-loops)# 目的：在消息传递 (propagate) 时，强制节点聚合自身上一层的特征，防止固有属性被邻居节点覆盖。
        edge_index 增加了 $N$ 条指向自身的边。维度从 [2, E] 变为 [2, E + N]
        ...
        # 全是 4-1-1 分类索引的虚拟自环特征 self_loop_attr
        ...
        self_loop_attr = self_loop_attr.to(edge_attr.device).to(edge_attr.dtype)
         ...
         # edge_attr 维度从 [E, 3] 变为 [E + N, 3]。
        edge_attr = torch.cat((edge_attr, self_loop_attr), dim=0)
      
对于图中的任意一条特定边（无论它是真实的化学键还是虚拟的自环），分别计算其三种离散属性对应的独立嵌入向量，
然后将这三个同维度的向量相加，压缩成一个统一的 edge_embeddings 向量。这个最终向量综合代表了该边（或自环）
的完整物理属性，随后被送入 propagate 参与消息传递。
维度为 [E + N, emb_dim]
        edge_embeddings = self.edge_embedding1(edge_attr[:,0]) + \
                          self.edge_embedding2(edge_attr[:,1]) + \
                          self.edge_embedding3(edge_attr[:,2])
触发消息传递
输入 [2, E+N], x [N, emb_dim], edge_attr [E+N, emb_dim] 进入底层引擎
        return self.propagate(edge_index, x=x, edge_attr=edge_embeddings)

message 函数来定义邻居节点传过来的“信息”是什么。
每一条边上的源节点特征 x_j 和边特征 edge_attr
...
假设当前批次包含 $B$ 个分子图，这 $B$ 个图被拼成了一张超大图。大图的总节点数为 $N$，总真实边数为 $E$。

pair_graph: PyG 的 Batch 对象，包含超大图的所有信息。
    def forward(self, pair_graph, cond):
        # GIN layer
        pair_graph.x: 节点特征张量，维度 [N, 5]。
...
多层卷积结束后，需要将大图（包含数千个节点）坍缩回 $B$ 个分子的独立表示。
特征降维。将深层节点特征从内部 emb 维度线性映射到目标特征维度。
[N, emb_dim] 映射为 [N, feat_dim]。
        h = self.feat_lin(h)

函数利用 batch 张量的累加计数逻辑，精准提取出每个独立分子图的最后一个节点（虚拟全局节点）。
本阶段输出 (h_pair)：从节点级表示变为图级表示，张量维度从 [N, feat_dim] 变为 [B, feat_dim]。
关键坍缩。函数利用 batch 张量精准索引
，提取出 Batch 中 $B$ 个分子分别对应的最后一个节点（虚拟全局节点）。数据实现从原子维度到分子维度的断崖式跨越。
